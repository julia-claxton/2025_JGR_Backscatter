...
In order to compare ELFIN data to our Geant4 modelling, we require a procedure to fit the fluxes recorded by ELFIN to the set of lookup tables generated with our Geant4 model. We first integrated each data segment described in Section \ref{sec:data-selection} over time, geometric factor, and energy to produce an electron distribution in energy/pitch angle space with units of electron counts. For distributions measured in the Southern hemisphere, the pitch angle look directions were mirrored about $90\mathdeg$ to match Geant4 lookup tables, which were simulated in the Northern hemisphere. At this point, all bins with upgoing pitch angles are discarded. Then, we perform a scaling procedure to account for the fact that ELFIN only measures part of the azimuthal distribution of downgoing electrons. To do this, we assume a uniform distribution of electron flux across all azimuthal angles. We then determine what fraction of the azimuthal distribution ELFIN measures compared to a ring of solid angle between the minimum and maximum pitch angles captured by ELFIN's field of view. At field-aligned pitch angles (i.e. $0\mathdeg$ and $180\mathdeg$), this fraction is $1$, as all azimuthal angles are measured by ELFIN at those locations. The scaling factor that converts ELFIN-measured counts to the total counts across a uniform azimuthal distribution for a given look direction is called $C$ and is defined by Equations \ref{equation:azimuth-correction-start} through \ref{equation:azimuth-correction-end}. Figure \ref{figure:azimuth-correction} shows a schematic diagram of the difference between the solid angle measured by ELFIN and the solid angle the simulation propagates.
...
To simulate the full downgoing electron distribution measured by ELFIN across all azimuthal angles, every downgoing pitch angle and energy bin in an ELFIN data segment locates the nearest pitch angle and energy for which a precomputed simulation beam exists, then adds a weight to that beam equal to the number of counts recorded in that bin multiplied by the value of $C$ at that bin's pitch angle. We repeat this process for every downgoing bin in a half spin of the satellite -- weighting the full spin results in a doublecounting of the downgoing distribution, as each half of the spin measures nearly the same pitch angles. After iteration over the half-spin downgoing ELFIN bins, we have a weighted set of beams for which we can look up the backscattered electron momenta.

\subsection{Virtual Detector Method}
To calculate a simulated backscatter rate from our weighted set of backscatter lookup tables, we utilize a virtual detector method where we construct a set of unit vectors with azimuthal angles equal to zero and pitch angles equal to the boresight angles of the anti-loss cone look directions in the ELFIN data. Each of these unit vectors represents the boresight angle of a virtual ELFIN detector. Each virtual detector finds the angle between all backscattered electron momenta in the weighted lookup tables and its boresight direction using the dot product. If the angle between the backscattered electron and the virtual detector is less than or equal to half the ELFIN electron detector field of view ($11.25\mathdeg$), the electron is sorted into one of ELFIN's energy channels and counted as a detection, weighted by the backscatter table's weight. This detects all backscattered electrons that lie within in a cone that has the same field of view as ELFIN's particle detector, separated into the same energy channels as ELFIN. The result of this virtual detection process is a simulated backscattered electron distribution on the same grid as the ELFIN data in energy/pitch angle space for the anti-loss cone. Then, similar to our process when cleaning the ELFIN data, any simulation energy/pitch angle bins with relative error greater than $50\%$ as defined in Equation \ref{equation:relative-error} are set to zero for parity with the calculation of backscatter on ELFIN. The simulated counts and simulated energy measured in the anti-loss cone are then summed and stored as a single value. The process of simulating ELFIN backscatter is illustrated in Figure \ref{figure:simulation-procedure}.
...
After simulating the backscatter for all data segments that were analyzed in Section \ref{section:data-backscatter}, the total anti-loss cone electron flux predicted by our simulation vs. the anti-loss cone count measured by ELFIN were plotted against each other in Figure \ref{figure:data-model-comparison}. We can see that the simulation has overall excellent agreement with the mean backscatter rate data, with a tendency to overestimate backscatter at very low fluxes. This overestimation at low fluxes is not altogether unexpected, as low fluxes cause ELFIN to measure artificially low backscatter ratios due to its noise floor as discussed in Section \ref{section:data-backscatter}. While we attempted to account for this effect by including a noise floor cutoff in our simulated data, this did not perfectly replicate ELFIN's noise floor. This is likely due to the fact that precisely replicating the number of counts in a given pitch angle and energy bin is a much more demanding task than replicating the counts in the entire loss cone over all energy channels, and the ability to replicate a backscatter distribution on a per-bin basis is limited by the pitch angle resolution that ELFIN measured the downgoing distribution with. Thus we conclude that the overestimation at low fluxes is unlikely to indicate a fundamental issue with our simulation, as the simulation performs excellently at all other flux levels. Additionally, we note the spread in the model's predictions compared to the data. We believe that this is in part due to a lack of sufficient pitch angle resolution near the loss cone edge, where backscatter dynamics change on the order of a single degree (see Section \ref{section:simulation}. Due to ELFIN's $22.5\mathdeg$ field of view, we cannot sufficiently resolve this sensitive region. By assuming all electrons measured by ELFIN had a pitch angle equal to the boresight pitch angle, we introduce an random inaccuracy that can account for the spread seen in the data-model comparison. Given our agreement with mean backscatter rates, we can conclude that pitch angle resolution limitation is likely a key factor in the spread between ELFIN-measured backscatter and our predicted backscatter.

\begin{figure}
  \noindent\includegraphics[width=\textwidth]{figures/data_model_comparison.png}
  \caption{
    Left: Anti-loss cone electron flux predicted by lookup tables derived from our Monte Carlo simulation vs. ELFIN-measured anti-loss cone electron counts for all ELFIN data segments that were simulated in Section \ref{section:data-backscatter}. The dashed white line indicates where predicted backscatter was equal to ELFIN-measured backscatter. Energy backscatter follows the same pattern and is not included for brevity. Right, top: $1$-dimensional histogram plotting total number of electrons in the anti-loss cone predicted by the simulation divided by the same quantity measured by ELFIN. Right, bottom: $1$-dimensional histogram plotting total energy in the anti-loss cone predicted by the simulation divided by the same quantity measured by ELFIN. This is based on \backscattertime of data.
  }
