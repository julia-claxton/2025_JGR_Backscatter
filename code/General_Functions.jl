...
function box_aspect!(aspect_ratio)
    ...
    plot!(aspect_ratio = square_aspect * aspect_ratio)
end

function normalize_1st_dimension_to_unity(dim2_edges, distribution)
    N = size(distribution)[1]
    Δx = diff(dim2_edges)
    normalized_distribution = copy(distribution)
    row_integrals = [sum(Δx .* distribution[i,:]) for i in 1:N]
    eachrow(normalized_distribution) ./= row_integrals

    # Verify integration
    this_should_be_1 = [sum(Δx .* normalized_distribution[i,:]) for i in 1:N]
    @assert all( (this_should_be_1 .≈ 1) .|| isnan.(this_should_be_1) )
