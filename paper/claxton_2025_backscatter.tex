...
Additionally, we note the spread in the model's predictions compared to the data. We believe that this spread is \st{due in part} \hl{largely due} to a lack of sufficient pitch angle resolution near the loss cone edge, where backscatter dynamics change on the order of a single degree (see Section \ref{section:simulation}). Due to ELFIN's $22.5\mathdeg$ field of view, we cannot sufficiently resolve this sensitive region. By assuming all electrons measured by ELFIN had a pitch angle equal to the boresight pitch angle, we introduce an random inaccuracy that may account for the spread seen in the data-model comparison. \hl{The atmospheric profile used in our simulation may account for some error, as neutral density was treated at a constant in our simulation. However, we believe error due to the atmospheric profile is negligible compared to the error due to instrument resolution. As we will show in Section {\ref{section:simulation}}, the vast majority of atmospheric interactions in EPP occur below $100$~km. Previous literature \mbox{\cite{marshall2018, cotts2011}} also supports this conclusion. The atmospheric neutral profile below $100$~km has very little variation with time (\mbox{\citeA{xu2020}}, Figure 3e). Magnetic field model may also introduce error. However, due to the generally static nature of the near-Earth magnetic field, errors in the magnetic field would manifest as a systematic bias in the residual distribution rather than a spread.} Given our agreement with mean backscatter rates, we can conclude that pitch angle resolution limitation is likely the primary factor in the spread between ELFIN-measured backscatter and our predicted backscatter.
