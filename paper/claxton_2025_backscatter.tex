...
To simulate the full downgoing electron distribution measured by ELFIN across all azimuthal angles, every downgoing pitch angle and energy bin in an ELFIN data segment locates the nearest pitch angle and energy for which a precomputed simulation beam exists, then adds a weight to that beam equal to the number of counts recorded in that bin multiplied by $C(\alpha_\text{bore})$. After iteration over all the downgoing ELFIN bins, we have a weighted set of beams for which we look up the backscattered electron momenta. To calculate the simulated backscatter rate, we construct a set of virtual ELFIN detectors with azimuthal angles equal to zero and pitch angles equal to the boresight angles of the anti-loss cone look directions in the ELFIN data. Each virtual detector then finds the angle between all backscattered electrons and its boresight direction. If this angle is less than or equal to half the ELFIN EPD field of view ($11.25\mathdeg$), the electron is counted as a detection. Backscattered electron detections are also classified by energy into ELFIN's $16$ energy channels. The result of this virtual detection process is a simulated backscattered electron distribution on the same grid as the ELFIN data in pitch angle/energy space for the anti-loss cone. Then, similar to our process when cleaning the ELFIN data, any bins with a predicted backscatter with relative error greater than $50\%$ as defined in Equation \ref{equation:relative-error} are set to zero for parity with the calculation of backscatter on ELFIN. The simulated counts and simulated energy measured in the anti-loss cone are then totaled up and stored as a single value. The process of simulating ELFIN backscatter is illustrated in Figure \ref{figure:simulation-procedure}.

\begin{figure}
  \noindent\includegraphics[width=\textwidth]{figures/simulation_procedure.png}
  \caption{
    Illustration of the procedure used to fit Geant4 lookup tables to ELFIN data. Solid lines mark the loss cone pitch angle and dashed lines mark the anti-loss cone pitch angle. \todo{1) ELFIN-recorded fluxes are integrated over time for a given data segment to produce differential directional fluence. 2) Each recording bin is multiplied by its energy span and instrument geometric factor to produce measured electron counts. 3) Each data bin in the loss cone is scaled by azimuthal field of view (see text) and assigns its number of counts as a weight to the nearest energy and pitch angle for which a backscatter lookup table exists. Circular markers indicate the location of these lookup tables, and their color indicates their weight in electron counts. Lookup tables with zero weight are not shown for clarity. 4) Backscatter lookup tables are summed by their total weights, de-scaled in azimuthal field of view (see text), and returned.}
  }
  \label{figure:simulation-procedure}
\end{figure}

After simulating the backscatter for all data segments that were analyzed in Section \ref{section:data-backscatter}, the total anti-loss cone electron flux predicted by our simulation vs. the anti-loss cone count measured by ELFIN were plotted against each other in Figure \ref{figure:data-model-comparison}. We can see that in general the simulation tends to overestimate backscattered fluxes by a factor of approximately $2$, with the overestimation reaching up to $2$ orders of magnitude at very low input fluxes.

very low = grazing. bins that are grazing were simulated but elfin has bad resolution there. look at where the bins lie, are we pulling mirrorers into the loss cone?

there's a pa bin that tends to lie around 60deg, this covers the region where fluxes spike all the way up in the trapped region. these trapped fluxes are then assigned to a beam that's in the loss cone where diffusion is much MUCH greater (see figure later), thus creating the overestimation in the loss cone.

generally low backscatter fluxes correspond to low lc fluxes, which is the more grazing population. it being worst there points towards issues with simulating grazers



\todo{Overestimation in the higher flux regimes indicates an overestimation of backscatter ratio for fluxes deep in the loss cone (i.e. nearly field-aligned). We further illustrate this point in Figure \ref{figure:data-model-comparison-examples}. Here, we have plotted examples of three data segments comparing ELFIN-recorded counts to model predictions. The dashed line is the pitch angle we use to define the anti-loss cone, including the standoff angle of half of ELFIN's FOV to ensure no contamination from the trapped region. The annotation ``\# ALC'' shows the number of counts recorded in the anti-loss cone for each distribution. In the first case (top row), the input fluxes are highly pitch angle scattered, with significant fluxes all the way to field-aligned pitch angles and the model overestimates the recorded backscatter. The bottom two rows show cases of slight underestimation and agreement respectively. Both rows show much less flux deep in the loss cone compared to the overestimation case. These examples are representative, and other events follow this general pattern. Thus, we conclude that our model is generally accurate in the trapped region and the beginning of the loss cone with a tendency to overestimate backscatter at more field-aligned input pitch angles.}

\begin{figure}
  \noindent\includegraphics[width=\textwidth]{figures/data_model_comparison.png}
  \caption{
    Anti-loss cone electron flux predicted by lookup tables derived from our Monte Carlo simulation vs. ELFIN-measured anti-loss cone electron counts for all ELFIN data segments that were simulated in Section \ref{section:data-backscatter}. The dashed white line indicates where predicted backscatter was equal to ELFIN-measured backscatter. This is based on \backscattertime of data.
  }
...
\begin{figure}
  \noindent\includegraphics[width=\textwidth]{figures/data_model_comparison_examples.png}
  \caption{
    Examples of three data segments comparing ELFIN-recorded counts to model predictions. The dashed line is the pitch angle used to define the anti-loss cone, including the standoff angle of half of ELFIN's FOV to ensure no contamination from the trapped region. The annotation "$\#$ ALC" shows the number of counts recorded in the anti-loss cone for each distribution. The top row shows a case where the model overestimates backscattered counts, the middle row shows slight underestimation, and the bottom row shows agreement.
  }
  \label{figure:data-model-comparison-examples}
\end{figure}
...
We then introduced an updated EPP backscatter model which was used to simulate all ELFIN data that was used in the backscatter analysis. This model showed good prediction of backscatter due to precipitating fluxes in the trapped region and the near loss cone, with a tendency to overestimate backscattered fluxes deep into the loss cone at more field-aligned pitch angles.
